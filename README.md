...
- **踏み台サーバーの扱い**: 本番環境の構築完了後は、ローカルのVagrant環境（踏み台サーバー）を停止・削除しても本番環境の動作に影響はありません。
- **インフラ設定の更新**: Ansibleのプレイブックや設定を変更して本番環境に反映させる場合は、再度 `vagrant up` で踏み台サーバーを起動し、`./provision.sh` を実行してください。
- **アプリケーションの更新**: アプリケーションコードを更新した場合は、GitHubへプッシュ（またはタグ付け）した後、再度 `./provision.sh` を実行することで、最新のソースコードが本番コンテナにデプロイされます。
...
### アプリケーションの取得バージョンについて

デプロイ時に取得されるソースコードの優先順位は以下の通りです：

1. **タグがある場合**: GitHubリポジトリにタグ（Git Tag）が付加されている場合は、最新のタグのバージョンを取得します。
2. **タグがない場合**: `main` ブランチの最新のソースコードを取得します。

※ バージョン管理を厳密に行う場合は、GitHub上でリリース（タグ付け）を行うことを推奨します。
